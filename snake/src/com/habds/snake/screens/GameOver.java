...
public class GameOver implements Screen, InputProcessor {
	...
	@Override
	public void show() {
		RootGame.getInstance().getRequestHandler().showAds(true);
		...
		stage.addAction(Actions.color(new Color(1, 1, 1, 0))); // задали макс
																// прозорість
		stage.addAction(Actions.color(new Color(1, 1, 1, 1), 0.5f)); // запустили
																		// екшн
		...
		ResourseManager.getInstance().font.setScale(0.6f * w / ResourseManager.getInstance().standartW, 0.6f * h
	...
	public void render(float delta) {
		...
		ResourseManager.getInstance().font.draw(ResourseManager.getInstance().batch, "level " + level, 0.35f * w,
				0.95f * h);
		ResourseManager.getInstance().fontSc.setScale(0.3f * w / ResourseManager.getInstance().standartW, 0.3f * h
				/ ResourseManager.getInstance().standartH);
		ResourseManager.getInstance().fontSc.draw(ResourseManager.getInstance().batch, "" + record, 0.76f * w,
				0.95f * h);
		ResourseManager.getInstance().fontSc.setScale(0.42f * w / ResourseManager.getInstance().standartW, 0.42f * h
				/ ResourseManager.getInstance().standartH);
		ResourseManager.getInstance().fontSc.draw(ResourseManager.getInstance().batch, String.valueOf(score), 0.3f * w,
				0.5f * h);
	...
	public ClickListener buttonClickListener = new ClickListener() {
		...
		public void clicked(InputEvent event, float x, float y) {
			...
			if (event.getListenerActor().getName() == "start") {
				...
				ctrlStart.addAction(Actions.sequence(Actions.color(new Color(toRGB(37, 89, 115)), 0.4f),
			...
			if (event.getListenerActor().getName() == "back") {
				...
				if (ifSound)
					ResourseManager.getInstance().buttonSound.play(1f);
				ctrlBack.addAction(Actions.sequence(Actions.color(new Color(toRGB(37, 89, 115)), 0.4f),
						Actions.run(new Runnable() {
							public void run() {
								dispose();
								rootGame.setScreen(rootGame.menuScreen);
